...
class Args:
    ...
    validation_visualizations: int = 4
    """number of val samples to visualize per epoch (only when observation_embedder == 'cnn' and track == True)"""
    ...
    for epoch in tqdm(range(args.num_epochs), desc="Training world model"):
        ...
        with torch.no_grad():
            ...
            for val_i, (X_batch, y_batch) in tqdm(
                ...
                if val_i == 0 and args.observation_embedder == "cnn" and args.track:
